...
previcao = pd.read_csv("datasets/previcao.csv")  # Dados de previsões geradas

#######################################################################################################################
# calcular_estatisticas(partidas)
# Calcula estatísticas básicas:
# - Percentual de vitórias dos mandantes, empates e vitórias dos visitantes.
# - Os 5 placares mais frequentes nas partidas realizadas.
#######################################################################################################################
def calcular_estatisticas(partidas):
    estatisticas = {}

    # Cálculo de vitórias, empates e derrotas
    ...
    # Determinação dos placares mais frequentes
...
# Gera um gráfico de barras mostrando a distribuição de resultados:
# - Vitórias dos mandantes, empates e vitórias dos visitantes.
# Retorna o gráfico em Base64 para renderização no HTML.
#######################################################################################################################
def gerar_grafico_barras(partidas):
    ...
    sns.countplot(data=resultados, x='Resultado', palette='viridis', legend=False)
...
# Retorna os 10 melhores times da classificação geral com base em pontos.
#######################################################################################################################
...
#######################################################################################################################
# gerar_classificacao()
# Retorna a tabela de classificação em formato HTML para exibição.
#######################################################################################################################
...
#######################################################################################################################
# gerar_previsoes()
# Treina modelos de aprendizado de máquina para prever os placares de partidas não realizadas.
# Salva os resultados em um arquivo CSV.
#######################################################################################################################
def gerar_previsoes():
    dados_realizados = pd.read_csv("datasets/Partidas_Realizadas.csv")
    dados_nao_realizados = pd.read_csv("datasets/Partidas_Nao_Realizadas.csv")
    ...
    X = dados_realizados[['Rodada', 'Mandante', 'Visitante', 'Estadio']]
    y = dados_realizados[['Placar_Mandante', 'Placar_Visitante']]
    ...
    dados_nao_realizados['Placar_Mandante_Previsto'] = modelo_mandante.predict(entradas_nao_realizadas).round().astype(int)
    dados_nao_realizados['Placar_Visitante_Previsto'] = modelo_visitante.predict(entradas_nao_realizadas).round().astype(int)

    # Salvando as previsões
    dados_nao_realizados.to_csv("datasets/Partidas_Previstas.csv", index=False)

#######################################################################################################################
# Rota /
# Rota principal do sistema. Renderiza o dashboard com estatísticas, gráficos e tabela de classificação.
#######################################################################################################################
